...
def get_y_true(task_name,data_dir):
    """ 
    ...
    """
    if task_name in ["sentihood_single", "sentihood_nli_m", "sentihood_qa_m", "sentihood_nli_b", "sentihood_qa_b","test_sentihood"]:
        true_data_file = os.path.join(data_dir,"test_NLI_M.tsv")
        ...
        true_data_file = os.path.join(data_dir,"test_NLI_M.csv")
    ...
    if task_name in ["sentihood_nli_m", "sentihood_qa_m"]:
        with open(pred_data_dir, "r", encoding="utf-8") as f:
            ...
            while s:
                ...
                s = f.readline().strip().split()
    elif task_name in ["sentihood_nli_b", "sentihood_qa_b","test_sentihood"]:
        count = 0
        tmp = []
        with open(pred_data_dir, "r", encoding="utf-8") as f:
            s = f.readline().strip().split()
...
def get_score(task_name,pred_data_dir,data_dir):

    result = collections.OrderedDict()
    if task_name in ["sentihood_single", "sentihood_nli_m", "sentihood_qa_m", "sentihood_nli_b", "sentihood_qa_b","test_sentihood"]:
        y_true = get_y_true(task_name,data_dir)
        y_pred, score = get_y_pred(task_name, pred_data_dir)
        aspect_strict_Acc = sentihood_strict_acc(y_true, y_pred)
        print("aspect_strict_Acc",aspect_strict_Acc)
        print("y_true",y_true[0:10],np.sum(y_true))
        print("y_pred",y_pred[0:10],np.sum(y_pred))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred_data_dir",
                        default="./results/sentihood_QA_B/absa_test_0.9357814440305127.txt",
                        type=str,
                        help="The pred data dir.")
    parser.add_argument("--data_dir",
                        default="/data/kou/glue_data/sentihood_QA_B/",
                        type=str,
                        help="The pred data dir.")
    args = parser.parse_args()

    result=get_score(args.task_name,args.pred_data_dir,args.data_dir)
