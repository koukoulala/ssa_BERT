...
class RobertaSeqAttention(BertPreTrainedModel):

    def __init__(self, config, num_labels, args):
        super(RobertaSeqAttention, self).__init__(config)
...
class RobertaTokenAug(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(RobertaTokenAug, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.layer_norm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class RobertaTokenAug_2(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, args):
        super(RobertaTokenAug_2, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.layer_norm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class RobertaForNSPAug(BertPreTrainedModel):
    r"""
    ...
    def __init__(self, config, args):
        super(RobertaForNSPAug, self).__init__(config)
        self.num_labels = config.num_labels
        print("num_labels", self.num_labels)
        self.roberta = RobertaModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.seq_cls = RobertaSeqAttention(config, self.num_labels, args)
        if args.share_weight == 1:
            self.token_cls = RobertaTokenAug(config, self.roberta.embeddings.word_embeddings.weight)
        else:
...
class RobertaForNSP_co(BertPreTrainedModel):
    r"""
    ...
    def __init__(self, config, args):
        super(RobertaForNSP_co, self).__init__(config)
        self.num_labels = config.num_labels
        self.roberta = RobertaModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if args.share_weight == 1:
