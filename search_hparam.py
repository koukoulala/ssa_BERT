...
from run_classifier_b_ratio import args, main
#from run_classifier_b_dy import args, main
...
class Job(object):
  ...
  def prepare(self):
    ...
    if os.path.exists(_args.log_dir) and os.listdir(_args.log_dir):
        print("Log dir ({}) already exists and is not empty.".format(_args.log_dir))
  ...
  while True:
    try:
      ...
      if new_done:
        ...
        # save to result_file
        result_file = os.path.join(_args.output_dir, "tune_results_"+args.task_name+"_ratio_4.26.txt")

        with open(result_file, 'w') as fout:
          for job in finished_jobs:
            if job.is_succeed:
              fout.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(job.job_id, job.result[0], job.result[1], job.result[2], job.result[3], job.settings))
            else:
