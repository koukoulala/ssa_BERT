...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, token_real_label=None):
        all_encoder_layers = []
    ...
    def from_state_dict(cls, config_file, state_dict, *inputs, **kwargs):
        ...
        for key in list(state_dict):
            if not key.startswith("bert."):
                logger.info("from_state_dict: Dropping {key}")
                del state_dict[key]
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True,
                token_real_label=None):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        if token_real_label is None:
            token_real_label = torch.ones_like(input_ids)
...
class BertSeqAttention(BertPreTrainedModel):
    ...
    def __init__(self, config, num_labels, args):
        super(BertSeqAttention, self).__init__(config)
        if config.hidden_size % config.num_attention_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (config.hidden_size, config.num_attention_heads))
...
class BertTokenAug(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(BertTokenAug, self).__init__()
        self.transform = BertPredictionHeadTransform(config)

        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder = nn.Linear(bert_model_embedding_weights.size(1), 2, bias=False)
        # self.decoder.weight = bert_model_embedding_weights
...
class BertTokenAug_2(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, args):
        super(BertTokenAug_2, self).__init__()
        self.transform = BertPredictionHeadTransform(config)

        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        decoder = nn.Linear(bert_model_embedding_weights.size(1), 2, bias=False)
        self.op = nn.ModuleList([copy.deepcopy(decoder) for _ in range(args.max_seq_length)])
        bias = nn.Parameter(torch.zeros(2)).cuda()
        self.op_bias = [bias.cuda() for _ in range(args.max_seq_length)]
...
class BertForNSPAug(BertPreTrainedModel):
    """BERT model with next sentence prediction head.

    """

    def __init__(self, config, num_labels, args):
        super(BertForNSPAug, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, token_real_label=None):
        ...
        if labels is not None:
            ...
            w = self.aug_loss_weight
            total_loss = (1 - w) * next_sentence_loss + w * aug_loss
            return seq_logits, aug_logits, next_sentence_loss, aug_loss, total_loss
        else:
            return seq_logits, aug_logits, aug_loss

class BertForNSP_co(BertPreTrainedModel):
    """BERT model with next sentence prediction head.

    """

    def __init__(self, config, num_labels, args):
        super(BertForNSP_co, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if args.share_weight == 1:
